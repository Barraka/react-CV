...
function OutputSchool2(props) {
    function deleteItem(e) {
        const target=e.currentTarget.parentElement.parentElement;
        const id=target.getAttribute('data-id');
        props.removeEducationItem(id);
    }
        ...
        return (
            <div className="itemSchool" key={x.id} data-id={x.id}>
                ...
                <div className='dates subsectionLine'>
                    <span className='completion2'>Completed: </span><span className='completion'> {x.completion}</span>
                </div>
