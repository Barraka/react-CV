...
class OutputWork extends Component {
    ...
    render() {
        const output = this.props.info.map(x=> {
            ...
            return (
                <div className="itemWork" key={x.id}>
                    ...
                    <div className='companyName'>{x.company}</div>
                    <span className='dateFrom'>From: {x.from}</span>
                    <span className='dateTo'>To: {x.to}</span>
                    <div className='tasks'>{x.tasks}</div>
                </div>
            )
